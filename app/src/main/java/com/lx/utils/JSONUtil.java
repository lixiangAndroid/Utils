...
public class JSONUtil {
     ...
     * <li>if jsonObject is null, return defaultValue</li>
     * <li>if jsonData {@link JSONObject#JSONObject(String)} exception, return defaultValue</li>
     * <li>return {@link JSONUtil#getJSONArray(JSONObject, String, JSONArray)}</li>
     * </ul>
     */
    ...
    /**
     * JSON转map tips：支持多组相同JSON数据的传输
     *
     * @param source key-value pairs json
     * @return <ul>
     * <li>if source is null or source's length is 0, return empty map</li>
     * <li>if source {@link JSONObject#JSONObject(String)} exception, return null</li>
     * <li>return {@link JSONUtil#parseJSONToList(String)}</li>
     * </ul>
     */
    public static List<Map<String, String>> parseJSONToList(String source) {
        try {

            JSONArray Data_jsonArray = new JSONArray(source);
            ...
            if (!Data_jsonArray.isNull(0)) {
                ...
                for (int i = 0; i < Data_jsonArray.length(); i++) {
                    ...
                    for (int j = 0; j < keys.size(); j++) {
                        map.put(keys.get(j), Data_jsonArray.getJSONObject(i).getString(keys.get(j)));
                    }
                    list.add(map);
                }
                return list;
     ...
     * 判断是否为Json格式
     */
    public static boolean isJSONFormat(String source) {
        try {
